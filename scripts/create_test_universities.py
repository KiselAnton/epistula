...
from test_university_helpers import (
    # Constants
    ROOT_EMAIL, ROOT_PASSWORD, TEST_UNIVERSITY_CODES, TEST_UNIVERSITY_NAMES,
    ...
    create_lecture_note, create_backup, restore_as_temp
)
...
def populate_university(token: str, uni_num: int):
    ...
    # Create faculties from shared config
    faculty_data = []
    for idx, fac_template in enumerate(FACULTIES):
        faculty = create_faculty(
            ...
            uni_id,
            fac_template["name"],
            fac_template["short_name"],
            f"{fac_template['short_name']}{uni_num}"
        )
        if faculty:
    ...
    for idx, faculty in enumerate(faculty_data):
        ...
        for i in range(PROFESSORS_PER_FACULTY):
            prof = create_user(
                ...
                password=DEFAULT_PROFESSOR_PASSWORD,
                role="professor",
                faculty_id=faculty_id
            )
            if prof:
        ...
        for subj_idx, subject in enumerate(subject_data):
            ...
            # For each student, add private notes to the first two lectures
            for student in students:
